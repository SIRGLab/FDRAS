...
def interleave_channels(err: torch.Tensor, main: torch.Tensor) -> torch.Tensor:
    # err/main shapes: [B, C, D, H, W]
    b, c, d, h, w = err.shape
    stacked = torch.stack([err, main], dim=2)  # [B, C, 2, D, H, W]
    return stacked.reshape(b, c * 2, d, h, w)  # err0,main0, err1,main1, ...


class ChannelFusionFixerUNet(nn.Module):
    """
    Encode error maps and main input to same channels, interleave channels,
    then decode delta map.
    """
    ...
    def __init__(self, in_channels: int = 2, err_channels: int = 2, base_channels: int = 8, norm: str = "INSTANCE"):
        ...
        # Error encoder (same channel sizes)
        ...
        # Decoder on fused channels (interleaved)
        self.up2 = UpBlock3d(c * 8, c * 4, norm)  # 8c -> 4c
        self.up1 = UpBlock3d(c * 4, c * 2, norm)  # 4c -> 2c
        self.out = nn.Conv3d(c * 2, 1, kernel_size=1)
    ...
    def forward(self, x_main: torch.Tensor, x_err: torch.Tensor) -> torch.Tensor:
        ...
        f1 = interleave_channels(ee1, e1)
        f2 = interleave_channels(ee2, e2)
        f3 = interleave_channels(ee3, e3)
...
__all__ = ["ChannelFusionFixerUNet"]
